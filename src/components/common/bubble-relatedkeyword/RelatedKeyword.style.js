...
export const StyledCircleItem = styled.div`
  ...
  left: ${(props) =>
    `calc(50% + ${props.x}px - ${parseInt(props.distance) * 0.5}px)`};
  top: ${(props) =>
    `calc(50% + ${props.y}px - ${parseInt(props.distance) * 0.5}px)`};
  animation: ${animate} ${(props) => props.time || "2s"}
    ${(props) => props.delay || "s"} ease-in-out infinite alternate;
  cursor: pointer;
`;
